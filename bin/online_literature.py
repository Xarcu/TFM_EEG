...
from antropy import sample_entropy, lziv_complexity
from scipy.stats import kurtosis, skew


"""
This script enables real-time or offline EEG classification using a pre-trained Common Spatial Patterns (CSP) model 
and a machine learning classifier. It supports both live streaming via Lab Streaming Layer (LSL) and offline processing of 
pre-recorded EEG data.

Key Features:
1. **Real-time EEG Processing**:
   - Connects to an active LSL stream to acquire live EEG data.
   - Filters and preprocesses incoming EEG signals.
   - Classifies the data using CSP-transformed features and a trained classifier.
   - Provides feedback in near-real-time.

2. **Offline EEG Processing**:
   - Reads EEG data from `.fif` files and simulates streaming to process it as if it were live.
   - Supports loading ground truth labels to compute classification accuracy.

3. **Baseline Handling**:
   - Allows recording a baseline for the EEG data to normalize signals during classification.

4. **Keyboard Monitoring**:
   - Enables users to toggle baseline recording or stop the program through keyboard inputs.

5. **Model Integration**:
   - Loads pre-trained CSP and classifier models to process and classify EEG data.

This script is designed for applications involving real-time brain-computer interfaces (BCIs) or the offline evaluation of BCI algorithms.
"""
...
def compute_features(data, sfreq):
    """
    Compute the 15 desired features for the given EEG data segment.

    Parameters:
        data (np.ndarray): EEG data segment (samples, channels).
        sfreq (float): Sampling frequency.

    Returns:
        feature_vector (list): List of 15 computed feature values.
    """
    feature_vector = []
    # Compute RMS
    ...
    for band, (low, high) in freq_bands.items():
        power = [
            np.sum(
                welch(data[:, ch], fs=sfreq, nperseg=min(128, len(data)))[1]
                [(low <= freq) & (freq <= high)]
            ) for ch, freq in enumerate(data.T)
        ]
        feature_vector.append(np.mean(power))
    
    # Compute additional features
    hurst = np.mean([sample_entropy(data[:, ch]) for ch in range(data.shape[1])])
    feature_vector.append(hurst)
    
    fractal = np.mean([
        np.log(len(data[:, ch])) - np.log(np.sum(np.diff(data[:, ch])**2))
        for ch in range(data.shape[1])
    ])
    feature_vector.append(fractal)
    
    kurt = kurtosis(data, axis=0)
    feature_vector.append(np.mean(kurt))
    
    skewness = skew(data, axis=0)
    feature_vector.append(np.mean(skewness))
    
    entropy = np.mean([sample_entropy(data[:, ch]) for ch in range(data.shape[1])])
    feature_vector.append(entropy)
    
    perm_entropy = np.mean([
        lziv_complexity(data[:, ch]) for ch in range(data.shape[1])
    ])
    feature_vector.append(perm_entropy)

    return feature_vector

def process_and_classify(data_source, classifier, feedback_duration=1.0, mode="online", sfreq=250, labels=None):
    """Process incoming data and classify using the extracted features."""
    ...
    while not stop_event.is_set():
        ...
        # Compute features
        features = compute_features(filtered_raw.get_data().T, sfreq)
        if len(features) != 15:
            print(f"Feature vector length mismatch: Expected 15, got {len(features)}")
            continue
...
def main(base_save_dir, classifier_model_path, mode, feedback_duration=1.0, offline_folder=None, labels_file=None):
    ...
    process_and_classify(data_source, classifier, feedback_duration, mode, sfreq, labels=labels)

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Real-time EEG Classification with Features and Trained Classifier")
    ...
    main("dummy_base_save_dir", args.classifier_model_path, args.mode, args.duration, args.offline_folder, args.labels_file)
