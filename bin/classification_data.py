...
import threading
import json
from mne.preprocessing import ICA  # Import ICA for artifact removal

"""
This script performs real-time or offline EEG classification using a pre-trained Common Spatial Patterns (CSP) model 
and a machine learning classifier. It supports real-time EEG streaming from Lab Streaming Layer (LSL) and offline 
processing of pre-recorded EEG data in `.fif` format.

Key Features:
1. **Real-time EEG Processing**:
   - Connects to an active LSL EEG stream.
   - Acquires EEG data in near real-time, preprocesses it, and applies artifact removal using ICA.
   - Uses CSP and a trained classifier to predict binary outcomes (e.g., LEFT vs. RIGHT).

2. **Offline EEG Processing**:
   - Reads EEG data from `.fif` files and simulates streaming to mimic real-time behavior.
   - Supports ground truth labels for evaluating classification accuracy.

3. **Preprocessing Pipeline**:
   - Filters EEG data (0.3–30 Hz) and applies notch filtering (50 Hz).
   - Performs Independent Component Analysis (ICA) to remove artifacts such as eye blinks and muscle movements.

4. **Classification Pipeline**:
   - Extracts CSP features from EEG data and classifies it using a pre-trained machine learning model.
   - Reports classification results and computes accuracy if ground truth labels are provided.

5. **Baseline Handling**:
   - Allows users to record baseline EEG data for normalization, improving classification reliability.

6. **Interactive Controls**:
   - Includes a keyboard interface to toggle baseline recording or stop the program.

7. **Mode Flexibility**:
   - **Online Mode**: Processes live EEG data from an LSL stream.
   - **Offline Mode**: Processes pre-recorded EEG data for testing and evaluation.

This script is ideal for applications in brain-computer interfaces (BCIs), enabling real-time classification or 
offline analysis of EEG data for binary classification tasks.
"""
...
def apply_ica(raw_data, sfreq):
    """Apply ICA to remove artifacts like eye blinks, muscle movements."""
    ica = ICA(n_components=20, random_state=97, max_iter=800)
    ica.fit(raw_data)
    
    # Optionally, you can plot ICA components to manually reject components that correspond to artifacts
    ica.plot_components()  # Visualize the components
    
    # Reject components (you can also automate this based on some heuristics)
    # For this example, we exclude components 0 and 1 as artifacts (you should inspect them)
    ica.exclude = [0, 1]
    cleaned_raw = ica.apply(raw_data)
...
def process_and_classify(data_source, csp, classifier, feedback_duration=1.0, mode="online", sfreq=250, labels=None):
    ...
    while not stop_event.is_set():
        ...
        cleaned_raw = apply_ica(filtered_raw, sfreq)

        # Reshape data for CSP transformation
        reshaped_data = cleaned_raw.get_data()[np.newaxis, :, :]  # Add a trial dimension
