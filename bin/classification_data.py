...
# def read_offline_xdf(folder_path):
#     """Read data from .xdf files in the specified folder."""
#     all_data = []
...
#                 print(f"No EEG stream found in {file_name}.")
#                 continue
...
#     if not all_data:
#         raise ValueError("No valid EEG data found in the specified folder.")
...
#         # Filter the data
#         filtered_raw = raw.copy().filter(l_freq=0.3, h_freq=30, fir_design='firwin', filter_length='auto')
...
#             if prediction == labels[label_index]:
...
#         data_source = inlet
#     elif mode == "offline":
#         if not offline_folder or not os.path.isdir(offline_folder):
#             print("Invalid offline folder specified.")
#             return

#         print(f"Reading offline data from {offline_folder}...")
#         try:
#             offline_data = read_offline_xdf(offline_folder)
#         except ValueError as e:
#             print(e)
#             return

#         sfreq = 250  # Sampling frequency (update if different in your data)
#         data_source = iter(chunk_offline_data(offline_data, feedback_duration, sfreq))

#         # Load labels if provided
#         labels = None
#         if labels_file:
#             # Skip the first row in the file if it contains a header
#             try:
#                 labels = np.loadtxt(labels_file, dtype=int, skiprows=1)
#             except Exception as e:
...
#     process_and_classify(data_source, csp, classifier, feedback_duration, mode, labels=labels)
...
#     parser.add_argument('-f', '--offline_folder', help="Path to the folder containing offline EEG data (required in offline mode)")
...
def main(base_save_dir, csp_model_path, classifier_model_path, mode, feedback_duration=1.0, offline_folder=None, labels_file=None):
    ...
    if mode == "online":
        ...
        # Debug: Check number of chunks generated
        num_chunks = 0
        for _ in chunk_offline_data(offline_data, feedback_duration, sfreq):
            num_chunks += 1
        print(f"Number of chunks generated: {num_chunks}")
        # Load labels if provided
        labels = None
        if labels_file:
            try:
